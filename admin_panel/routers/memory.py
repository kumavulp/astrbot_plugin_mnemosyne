...
def setup_memory_routes(app, plugin_instance):
    ...
    async def edit_memory(memory_id: str, request: Request):
        try:
            ...
            result = await memory_service.edit_memory_content(memory_id, new_content)
            return result
        except HTTPException:
            raise
        except Exception as e:
    ...
    async def import_memories(request: Request):
        try:
            ...
            if not memories:
                raise HTTPException(status_code=400, detail="memories 列表不能为空")
            if not session_id:
                raise HTTPException(status_code=400, detail="session_id 不能为空")
            result = await memory_service.import_memories(memories, session_id)
            return result
        except HTTPException:
            raise
        except Exception as e:
