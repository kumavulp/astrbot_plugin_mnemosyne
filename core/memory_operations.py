...
async def _get_summary_llm_response(
    plugin: "Mnemosyne", memory_text: str, session_id: str = ""
) -> LLMResponse | None:
    """
    请求 LLM 进行记忆总结。
    ...
    # [fix] 基于session_id判断群聊/私聊（不再用正则，避免私聊误判）
    _is_group_context = "GroupMessage" in session_id if session_id else False
            ...
            summary_contexts.append(
                {
                    "role": "system",
                    "content": (
                        "注意：本段对话来自群聊。格式 user(数字ID) 表示不同用户，"
                        "请在总结开头标注【群聊】，并区分各用户的发言，不要混为一人。"
                        "仅总结与AI助手直接相关的互动，忽略群员之间的闲聊。"
                    ),
                }
            )
        else:
...
async def handle_summary_long_memory(
    ...
    try:
        # 1. 请求 LLM 进行总结
        llm_response = await _get_summary_llm_response(plugin, memory_text, session_id=session_id)
        if not llm_response:
            return False
...
async def store_manual_memory(
    ...
    logger.info(f"[DEBUG] store_manual_memory 开始，content长度={len(normalized_content)}")
    logger.info(f"[DEBUG] milvus_manager={plugin.milvus_manager}, connected={plugin.milvus_manager.is_connected() if plugin.milvus_manager else 'N/A'}")
    logger.info(f"[DEBUG] embedding_provider={plugin.embedding_provider}")
    
    if not await _check_summary_prerequisites(plugin, normalized_content):
        logger.error("[DEBUG] _check_summary_prerequisites 返回 False!")
        return False

    target_session_id = session_id or event.unified_msg_origin
    logger.info(f"[DEBUG] session_id={target_session_id}, valid={validate_session_id(target_session_id) if target_session_id else False}")
    ...
    logger.info(f"[DEBUG] persona_id={target_persona}")

    if not plugin.embedding_provider:
        logger.error("手动记忆写入失败：Embedding Provider 不可用。"); plugin._last_write_error = "Embedding Provider不可用"
        return False
    ...
    try:
        ...
        logger.error(f"手动记忆写入失败：获取 Embedding 异常: {e}", exc_info=True); plugin._last_write_error = f"Embedding异常: {e}"
        return False

    if not embedding_vector:
        logger.error("手动记忆写入失败：Embedding 结果为空。"); plugin._last_write_error = "Embedding结果为空"
        return False
